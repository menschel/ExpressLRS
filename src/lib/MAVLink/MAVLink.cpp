...
/*
 * Adapted from Ardupilot Common and Math Lib
 */
...
ftype longitude_scale(int32_t lat);
ftype longitude_scale(int32_t lat)
{
    ftype scale = cos(lat * (1.0e-7 * DEG_TO_RAD));
    return MAX(scale, 0.01);
}

// return horizontal distance in meters between two locations
ftype get_distance(const int32_t lat1, const int32_t lon1, const int32_t lat2, const int32_t lon2);
ftype get_distance(const int32_t lat1, const int32_t lon1, const int32_t lat2, const int32_t lon2)
{
    ftype dlat = (ftype)(lat2 - lat1);
    ftype dlng = ((ftype)diff_longitude(lon2, lon1)) * longitude_scale((lat1+lat2)/2);
    return sqrt(dlat*dlat + dlng*dlng) * LATLON_TO_M;
}


// return bearing in radians from location to loc2, return is 0 to 2*Pi
ftype get_bearing(const int32_t lat1, const int32_t lon1, const int32_t lat2, const int32_t lon2);
ftype get_bearing(const int32_t lat1, const int32_t lon1, const int32_t lat2, const int32_t lon2)
{
    const int32_t off_x = diff_longitude(lon2,lon1);
    const int32_t off_y = (lat2 - lat1) / longitude_scale((lat1+lat2)/2);
...
int32_t get_bearing_to(const int32_t lat1, const int32_t lon1, const int32_t lat2, const int32_t lon2);
int32_t get_bearing_to(const int32_t lat1, const int32_t lon1, const int32_t lat2, const int32_t lon2)
{
    return int32_t(get_bearing(lat1, lon1, lat2, lon2) * DEGX100 + 0.5);
}
...
#define GPS_ALTMSL_OFFSET           22
    ...
    // Altitude MSL in dm
    gps_status |= prep_number(roundf(alt_msl_mm * 0.01f),2,2)<<GPS_ALTMSL_OFFSET;
    return gps_status;
}
 ...
 */
static uint32_t format_home(float distance_to_home_m, float altitude_above_home_m, float bearing_to_home_centideg);
static uint32_t format_home(float distance_to_home_m, float altitude_above_home_m, float bearing_to_home_centideg)
{
...
#define HOME_BEARING_OFFSET         25
    ...
    uint32_t home = prep_number(roundf(distance_to_home_m), 3, 2);
    ...
    home |= (((uint8_t)roundf(bearing_to_home_centideg * 0.00333f)) & HOME_BEARING_LIMIT)<<HOME_BEARING_OFFSET;
    // altitude between vehicle and home_loc in 0.1 meters.
    home |= prep_number(roundf(altitude_above_home_m*10), 3, 2)<<HOME_ALT_OFFSET;
    return home;
}
...
#define ATTIANDRNG_RNGFND_OFFSET    21
    uint32_t attiandrng = ((uint16_t)roundf((roll_rad * RAD_TO_DEG * 100 + 18000) * 0.05f) & ATTIANDRNG_ROLL_LIMIT);
    attiandrng |= ((uint16_t)roundf((pitch_rad * RAD_TO_DEG * 100 + 9000) * 0.05f) & ATTIANDRNG_PITCH_LIMIT)<<ATTIANDRNG_PITCH_OFFSET;
    return attiandrng;
}
...
static uint32_t format_terrain(float altitude_terrain)
{
    float terrain_alt_m = 0;
    if (!isnan(altitude_terrain)){
        terrain_alt_m = altitude_terrain;
    }
    uint32_t value = prep_number(roundf(terrain_alt_m * 10), 3, 2);
    return value;
}
...
#define CRSF_AP_CUSTOM_TELEM_SINGLE_PACKET_PASSTHROUGH (0xF0)
    ...
    crsfpassthrough.p.appid = htole16(appid);
    crsfpassthrough.p.data = htole32(data);
...
// We use altitude relative to home for GPS altitude, by default, but we can also use GPS altitude if USE_MAVLINK_GPS_ALTITUDE is defined
                ...
                if (home_latitude != 0){
                    bearing = get_bearing_to(gps_int.lat, gps_int.lon, home_latitude, home_longitude);
                    distance_to_home = get_distance(gps_int.lat, gps_int.lon, home_latitude, home_longitude);
                }
                ap_send_crsf_passthrough_single(0x5004, format_home(distance_to_home, relative_alt/1000, bearing));
                break;
            }
